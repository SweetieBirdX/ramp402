...
Three `.env` files, one per component. Copy each example and fill it in:
...
The one value you must have before anything else works is `OPERATOR_SECRET_KEY` — the key baked
into the deployed contract. It is not generated by any script, deliberately: a fresh operator would
produce a system that looks configured and fails on every paid call with `NotOperator`. Ask the team
for it. `CONTRACT_ID` is in the "## Contract" section above.
...
It funds the platform pool, reads the USDC issuer from the anchor's `stellar.toml` (never
hardcoded), adds the pool's trustline, funds the treasury, registers a demo endpoint against the
deployed contract, and caches it in SQLite so `/proxy/:slug` works the moment the gateway starts.
It prints a PASS/FAIL line per step and ends with an environment block to paste into
`gateway/.env`.

It is **idempotent**: accounts already in your environment are reused, funding and trustlines are
no-ops when already done. Re-run it as often as you like. Secrets are printed to the terminal once
and never written to a file in the repo — place them yourself.
...
Deletes the SQLite cache, recreates it from `gateway/schema.sql`, and registers a fresh demo
endpoint, so the dashboard starts empty. It does **not** redeploy the contract, change any key, or
undo anything on chain — the ledger is permanent and the cache is rebuildable. Set
`DEMO_SELLER_SECRET_KEY` in your root `.env` to keep the same demo seller across resets.

### 7. Check it works

```bash
cd contract && cargo test                        # 23 unit tests, offline
npx tsx scripts/smoke-contract.ts                # the deployed contract, on testnet
cd gateway && npm test                           # gateway suite
```

> **What has actually been walked through:** steps 1–4, 6 and 7 were run end to end on a clean
> checkout while writing this. Step 5 has not — the gateway's routes and the frontend's pages are
> still being built, so `npm run dev` starts a server that does not yet serve the full API. Those
> two commands are the intended entry points and will be verified as part of integration.
